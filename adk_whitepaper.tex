...
\title{Aidos Kuneen \\ --- Blockless and Anonymous Cryptocurrency for Quantum Era ---}
...
\begin{abstract}
		In this white paper we introduce a new cryptocurrency \emph{Aidos Kuneen}. Aidos Kuneen focuses on 
		blockless, privacy, decentralization and scalability for post quantum
		era with no fee. 
		
		Aidos Kuneen employed \emph{iMesh}.
		All transactions are directly referred by each others and forms  \emph{DAG (Directed Acyclic Graph)} structure.
		`SPECTRE' makes full-nodes possible to determine which transactions are legit by the DAG structure.
		SPECTRE is resilient to attackers  with up to 50\% of the computational power, not 33\%.
		For quantum era we employ a hash-based signature `XMSS'.
		Signature size and public key size of XMSS are relatively small.
		Zero-knowledge proof for post-quantum `ZKBoo (ZKB++)` is utilized for 
		anonymous transfers called \emph{AKShuffle}.
		For IoTs we introduce cooperative Proof of Work called \emph{coPoW}. Each senders can do PoW together by coPoW.
		Finally we simulate the number of leaves in iMesh. We determine the minimum number of reference transactions 
		to converge the DAG.
		This leads to faster confirmation time of transactions with minimum affects to the transaction size.
...
Bitcoin, which is invented by Satoshi Nakamoto~\cite{btc} in 2008, is widely used
in all over the world now. In Bitcoin, transactions are stored
into block-chains which are confirmed by miners  with Proof of Work (PoW).
Then miners get transaction fee from senders of transactions.
Block-chains are created periodically per around 10 minutes.
A transaction consists with inputs and outputs with these addresses,
and the inputs refer outputs of previous transactions to be spent.
Transactions are signed in Elliptic Curve Digital Signature Algorithm (ECDSA) by the address owner 
to reveal that the owner is legit.
