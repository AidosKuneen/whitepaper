...
\item Unlike Bitcoin, in which  only PoWs in blocks are required to be verified, in Aidos Kuneen PoWs in all transactions must be verified. Apparently there are much more transactions than blocks, therefore 
Aidos Kuneen requires more power for verification of PoW. Thus a faster verification of Cuckoo Cycle meets the requirement.
...
the system will not allow partial withdrawals of the user's balance because withdraw is forbidden for more than twice with same shuffle address.
...
This scenario is not necessarily the result of malicious intent, infact, it could be expected to occur quite frequently during normal network operation.
