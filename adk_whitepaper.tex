...
In order to prevent this potential attack vector, we introduce a mechanism known as the `Federated Byzantine Agreement ' (as discussed in Section~\ref{sec:fba}), 
...
\subsection{The Federated Byzantine Agreement Mechanism (FBAM)}
\label{sec:fbam}
...
trust the SPECTRE consensus mechanism during the early stages of network growth. Thus we must introduce the `Fedarated Byzantine Agreement Mechanism (FBAM)', a temporary 
system which utilises `The Ripple Protocol Consensus Algorithm' with a semi-trusted model as detailed in~\cite{ripple}, in order to validate transactions until 
such time as the SPECTRE consensus mechanism can take over.

Within the Ripple Protocol the Federated Byzantine Agreement method is used in order to confirm the validity of all transactions. A similar system referred to as the `FBAM' is implemented in Aidos Kuneen to serve the same purpose.
With the FBAM in place, a group of trusted nodes periodically confirm the validity of randomly selected transactions from within iMesh. A transaction that has been randomly selected for verification is known as a `Statement'. 
In order to determine if the Statement is true or false, the trusted nodes examine the Statement's `timestamp' in iMesh.

The process of verifiying a Statement is as follows:

\vspace{-0.5\baselineskip}
\begin{enumerate}
	\setlength\itemsep{0em}
	\item Each full-node defines the addresses of trusted nodes within their trusted list.
	\item One of the full-nodes selects a candidate transaction (which now becomes known as a Statement) for review and broadcasts it to the trusted nodes within their trusted list.
	...
	\item Each trusted node broadcasts their result to the full-nodes that reference them.
	\item If a full-node does not receive a miniumum of \(T\%\) of `yes' votes from the nodes on its trusted list within a certain period, the full-node will deem the Statement to be false and it will be discarded.
	\item Each trusted node increases \(T=T+10\%\).
	\item Step 5. is repeated
	\item The final round of agreement requires a minimum of \(T=80\%\) of a full-node's trusted nodes in agreement in order to accept the Statement as true.
\end{enumerate}

Once a Statement has been accepted by the network as being true, it becomes a reference point (ie: milestone) for the validity of other transactions.

Note: In order to be accepted, Statements must have a valid transaction structure and must exist within iMesh. Conflicting transactions are also elegible for elevation and consideration as Statements, this is because the validation of a Statement 
considers the `timestamp' of the transaction, not the transaction history.

Figure~\ref{fig:fba} illustrates how Statements act as milesontes for other transactions.
Statements which the FBAM deems to be legitimate are subsequently regarded as `True Statements' by the full-nodes 
and any conflicting transactions are discarded locally by each full node. Transactions that are referred to by a True Statement become known as 
Finalised Transactions.
The numbered transactions in Figure~\ref{fig:fba} show how transactions are finalised by the True Statements that reference them, e.g. the transactions designated with #2 are finalised
by Statement #2.

The rules employed by the FBAM in order to determine the validity of conflicting transactions are as follows.
...
The Federated Byzantine Agreement Mechanism requires a minimum of 80\% agreement between trusted nodes in order to accept a Statement as true, and by extension, 
20\% or more in order to reject a false Statement.
While it is acknowledged that introduction of The FBAM does directly contradict Aidos Kuneen's ultimate 
vision of a completely decentralised cryptocurrency, the FBAM, for the reasons outlined previously, is accepted to be a
...
the presence of the FBAM allows the realisation of this vision from day one, with no need to resort to alternative, `incentivised' 
