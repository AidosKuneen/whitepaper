...
In order to prevent this potential attack vector, we introduce a mechanism known as the `The Arbiter', discussed in Section~\ref{sec:arbiter}, 
to confirm transactions between trusted full-nodes defined by each full-nodes while iMesh is still in its infancy.
...
could realistically command the majority of the total network hash power. It is for this reason that we are therefore unable to trust the
SPECTRE consensus mechanism during the early stages of network growth. Thus we must introduce `The Arbiter', a temporary system which 
uses Ripple's `federated byzantine agreement'  explained in~\cite{ripple} to validate transactions until such time as the SPECTRE consensus mechanism can take over.
Each trusted nodes agree with one of transactions in iMesh along with a serial number  as the `timestamp'(The Arbiter).
...
\begin{enumerate}
	\setlength\itemsep{0em}
	\item Each node defines addresses from trusted nodes in their trusted list.
	\item One of trusted nodes broadcast one Arbiter to others.
	\item Each server then amalgamates the Arbiter of all servers on its trusted list, and votes on the veracity
	of the Arbiter.
	\item Transactions that receive more than a minimum percentage of `yes' votes are passed on to the next
	round, if the Arbiter doesn't receive enough votes it will be discarded
	\item The final round of consensus requires a minimum
	percentage of 80\% of a server's trusted nodes agreeing on the Arbiter.
\end{enumerate}

Transactions which The Arbiter deems to be 
...
This federated byzantine agreement needs over 80\% of agreement of trusted nodes not to stop confirmation, and needs over 20\% of agreement not to legitimate fraudulent transactions.
While it is acknowledged that the  byzantine agreement suffers from these restrictions which does directly contradict Aidos Kuneen's ultimate 
...
the presence of The Arbiter allows the realisation of this vision from day one, with no need to resort to alternative, `incentivised' mechanisms for early network protection.
