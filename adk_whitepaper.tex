...
\subsection{The Federated Byzantine Agreement (FBA)}
\label{sec:fba}
...
trust the SPECTRE consensus mechanism during the early stages of network growth. Thus we must introduce the `Fedarated Byzantine Agreement (FBA)' a temporary 
system which utilises `The Ripple Protocol Consensus Algorithm' with semi-trusted model as detailed in~\cite{ripple}, in order to validate transactions until 
such time as the SPECTRE consensus mechanism can take over.

While in Ripple Protocol the FBA is used in order to agree with validation of all transactions,
with the FBA for Aidos Kuneen in place, a group of trusted nodes periodically agree a randomly selected transaction from within iMesh, known as a `Statement' in order to determine 
the `timestamp' in iMesh.

The process of agreement a Statement is as follows:

\vspace{-0.5\baselineskip}
\begin{enumerate}
	\setlength\itemsep{0em}
	\item Each full-node defines the addresses of trusted nodes within their trusted list.
	\item One of the full-nodes selects a candidate Statement (i.e.\ a transaction) for agreement and broadcasts it to the trusted nodes within the trusted list.
	\item Each trusted nodes initialize \(T\) to 50\%.
	%\item Each trusted nodes then elevates the candidate statement to their trusted nodes within their trusted list.
	\item Each trusted nodes reviews the legitimacy of the candidate statement.
	\item Each trusted nodes broadcasts their own results to all full-nodes.
	%\item Each of the trusted nodes applies the same ruleset to to its own transaction history in order to determine the legitimacy of the statement.
	%\item Once \(T\) \% of  trusted nodes have arrived at a decision on the legitimacy of the statement, the verdict of a particular trusted node is relayed
	%to all the full-nodes.
	...
	\item If a full-node doesn't receives more than \(T\%\) of `yes' votes for the candidate statement in a certain period, the candidate statement will be discarded.
	\item Each trusted nodes increase \(T=T+10\%\).
	\item Go to 5.
	\item The final round of agreement requires a minimum percentage of \(T=80\%\) of a server's trusted nodes in agreement in order to accept the Statement.
\end{enumerate}

Note:
Statements  must have a valid transaction structure and must be within iMesh, but may be one of  conflicting transactions. This is because the statement is
only for  a `timestamp'  in iMesh.

Figure~\ref{fig:fba} shows a example of validation of transactions.
Transactions which the FBA deems to be legitimate are subsequently regarded as `finalized transactions' by the full-nodes 
and any conflicting transaction are discarded locally by each full nodes.
The txs including number in Figure~\ref{fig:fba} shows that the txs are finalized, e.g. the txs with number 1 is finalized
by the statement with serial number 1.
The rules employed by The FBA in order to determine the validity of conflicting transactions are as follows.
...
\begin{itemize}
	\item The transaction which is finalized by youngest statement is deemed to be valid, and all others are considered to be invalid.
...
This federated byzantine agreement method requires a minimum of 80\% agreement between trusted nodes in order to accept a Statement as legitimate, and by extension, 
20\% or more in order to reject a fraudulent Statement.
...
vision of a completely decentralised cryptocurrency, the BFT, for the reasons outlined previously, is accepted to be a
...
the presence of the BFT agreement allows the realisation of this vision from day one, with no need to resort to alternative, `incentivised' 
