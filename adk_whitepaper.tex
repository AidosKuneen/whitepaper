...
\begin{enumerate}
	...
	\item One of the full-nodes selects a candidate transaction for review and broadcasts it to the trusted nodes within their trusted list.
	\item Each trusted node initialises \(T\) to 50\%.
	\item Each trusted node reviews the legitimacy of the transaction based on its local history.
	\item If a trusted node deems a transaction to be legitimate, it then broadcasts a `yes' vote to the full-nodes by which it is referenced. If the transaction is deemed to be fraudulent it is subsequently discarded.
	\item If a full-node does not receive a minimum of \(T\%\) of `yes' votes from the nodes on its trusted list within a certain period, the transaction is discarded.
	\item Each trusted node increases \(T=T+10\%\).
	\item Step 5. is repeated
	\item The final round requires a minimum of \(T=80\%\) of a full-node's trusted nodes in agreement in order to accept the the transaction as being legitimate and elevate it to the status of a Statement.
\end{enumerate}

Note: In order to be accepted, Statements must have a valid transaction structure and must exist within iMesh.
...
AKConsensus requires a minimum of 80\% agreement between trusted nodes in order to accept a Statement, and by extension, 
20\% or more in order to reject an invalid Statement.
