...
We use Cuckoo Cycle mentioned in \cite{cuckoo} as PoW. 
Cuckoo Cycle is based on 
inserting items in a Cuckoo hashtable and it naturally leads to cycle formation in random bipartite graphs.
A Cuckoo hashtable consists of two
...
is reached.The latter is bound to happen once cycles have formed in the Cuckoo graph.
This is a bipartite graph with a node for each location and an edge for every inserted key, connecting the two
locations it can reside at.

\begin{figure}[ht]
	\begin{center}
	\includegraphics[width=80mm]{cuckoo.png}
	  \caption{Cuckoo Cycle}
    \label{fig:cuckoo}
	\end{center}
 \end{figure}

 The left diagram in Figure \ref{fig:cuckoo} shows the directed cuckoo graph on \( N = 8 + 8 \) nodes
 after adding edges (2, 15), (4, 9), (8, 5), (4, 15), (12, 11), (10, 5) and (4, 13) .
 In order to add the 8th edge (10, 11), we follow the paths \( 10 \rightarrow 5 \rightarrow 8 \) and
 \( 11 \rightarrow 12 \) to find different roots 8 and 12. 
 Since the latter path is shorter, we reverse it to \( 12 \rightarrow 11\) so
 we can add the new edge as (\( 11 \rightarrow 10\) ), resulting in the middle diagram. In order to add to 9th edge
 (10, 13) we now find the path from 10 to be the shorter one, so we reverse that and add the new edge
 as (\( 10 \rightarrow 13\)), resulting in the right diagram. When adding the 10th edge (8, 9), we find the paths
 \( 8 \rightarrow 5 \rightarrow 10 \rightarrow 13 \rightarrow 4  \rightarrow 15 \rightarrow 2 \) and
 \( 9 \rightarrow 4 \rightarrow 15 \rightarrow 2 \) 
 with equal roots. In this case, we can compute
 the length of the resulting cycle as 1 plus the sum of the path-lengths to the node where the two paths
 join. In the diagram, the paths join at node 4, and the cycle length is computed as 1 + 4 + 1 = 6.

The reason we choise Cuckoo Cycle as PoW is as follows.
